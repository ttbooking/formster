    ...
    <select {{ $attributes }} name="{{ $property->variableName }}" @disabled(! $property->writable)>
        @foreach ($timezones as $region => $timezone)
            @if (is_string($timezone))
                <option value="{{ $timezone }}" @selected($timezone === old($property->variableName, $object->{$property->variableName}->getName()))>
                    {{ $timezone }} ({{ now($timezone)->getOffsetString() }})
                </option>
            @else
                <optgroup label="{{ $region }}">
                    @foreach ($timezone as $location)
                        <option value="{{ "$region/$location" }}" @selected("$region/$location" === old($property->variableName, $object->{$property->variableName}->getName()))>
