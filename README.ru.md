...
> Атрибут `#[Aura]` включает для модели мягкий режим политик доступа: пока политики не написаны, все свойства видимы и редактируемы. Без атрибута (и без политик) штатный `Gate` запретит доступ, и форма окажется пустой. Подробнее — в разделе [Контроль доступа](#контроль-доступа-политики).
...
| Сущность                                  | Назначение                                                                                                                                                                                                                                                                    |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`Aura`**                                | «Аура» класса: краткое (`summary`) и полное (`description`) описание, коллекция свойств (проиндексирована по имени), политики по умолчанию (`viewPolicy`, `updatePolicy`) и фильтры свойств `include`/`exclude`. Одновременно является PHP-атрибутом уровня класса `#[Aura]`. |
 ...
 * @property int $age
 * @property string|null $bio          Может быть null
...
Атрибут `#[Aura]` умеет ещё и фильтровать, какие свойства попадут в форму: `include` — белый список (не указан — проходят все), `exclude` — чёрный:
...
Фильтр применяется на этапе финализации, поэтому вырезает и свойства, найденные другими парсерами — PHPDoc или рефлексией. При слиянии аур (агрегация, наследование) оба списка объединяются; `exclude` всегда сильнее `include`.

Атрибуты подхватываются и с родительских классов: парсер `aura` проходит всю цепочку наследования и сливает метаданные в одну ауру — общие описания и фильтры можно держать в базовом классе. При конфликте побеждает потомок: метаданные, объявленные ближе к самому классу, перекрывают родительские.
...
| ---------------------- |---------------------------------------------------------------------------------------------------------------------------|
| `aura`                 | PHP-атрибуты `#[Aura]` / `#[AuraProperty]` — с самого класса и со всех его предков                                        |
| `reflection`           | Нативные типизированные публичные свойства                                                                                |
...
> **Порядок важен:** парсеры, идущие позже в списке, имеют приоритет. Одноимённые свойства сливаются поле за полем (`AuraProperty::merge()`): каждый элемент метаданных (тип, описание, политика и т.д.) берётся из более позднего парсера, если он там задан, иначе сохраняется значение более раннего. Правила валидации сливаются по нотации `'...'` (см. [Валидация](#валидация)).
...
Обработчик (`PropertyHandler`) связывает тип свойства с виджетом, правилами валидации и логикой записи. Контракт:
...
Конструктор валидирует формат (`/^#[0-9a-fA-F]{6}$/`) и при ошибке бросает `InvalidArgumentException`.
 ...
 *
 * Только пояса РФ (двухбуквенный ISO-код страны):
...
> Поле `list<File>` рендерится с атрибутом `multiple`, и форма получает корректный `enctype`, однако серверная обработка нескольких загруженных файлов пока не реализована — `FileHandler` в текущей версии пропускает массивы файлов.
...
Где и как выполняются проверки:

- **показ строки таблицы** — `viewPolicy` класса и свойства вместе, аргументы `[$object, $имяСвойства]`;
- **редактируемость строки** и **обработка отправки** — `updatePolicy` класса и свойства вместе, те же аргументы;
- **кнопка «Сохранить»** — только `updatePolicy` уровня класса, аргумент — только объект (без имени свойства).

Метод политики получает модель и имя свойства:
...
> **Важно:** мягкий режим распространяется только на классы, помеченные атрибутом `#[Aura]` — на самом классе или на любом из его родителей. Модель, описанная только PHPDoc-аннотациями (без атрибута), под мягкий режим **не попадает**: если для неё не определена политика, штатный `Gate::check()` запретит доступ, и свойства не будут показаны. Поэтому либо помечайте модели атрибутом `#[Aura]`, либо пишите политики.
...
```
formster.{model|object}.{alias}.{имя_свойства_в_snake_case}
```

Для вариантов перечислений:
...
Если перевод не найден, используется текст PHPDoc-аннотации; если и его нет — описание генерируется из имени свойства (`Str::headline`).

---

## Алиасы

`alias` — это ключ, под которым модель/перечисление фигурирует в строках локализации. По умолчанию он вычисляется из имени класса (с отбрасыванием namespace `App\Models\` / `App\Enums\` и переводом остатка в `snake_case`). Зафиксировать собственный алиас можно атрибутом `#[Alias]`:
...
return [
    ...
    // false — мягкий режим (разрешить доступ при отсутствии политики/метода),
    // true — обычное поведение Laravel Gate.
...
Команда интерактивна: при отсутствии аргументов спросит имя и предложит выбрать тип из каталога `app/Formster/Types`. Класс создаётся в пространстве имён `App\Formster\Handlers`.
...
Зарегистрируйте обработчик в `config/formster.php` (в массиве `property_handlers`, до `FallbackHandler`) и создайте Blade-виджет `form.money`.
...
CI прогоняет матрицу PHP 8.2–8.5 × Laravel 12.17 / 13.0 (prefer-lowest и prefer-stable); комбинация PHP 8.2 × Laravel 13 исключена.
