...
> Если среди свойств есть файл или изображение, форма автоматически получает `enctype="multipart/form-data"`. Исключение: если файловые поля объявлены только как `list<File>`, автоопределение не срабатывает — добавьте атрибут `enctype` вручную.
